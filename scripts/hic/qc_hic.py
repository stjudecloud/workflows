...
def get_args():
    parser = argparse.ArgumentParser(description="QC a Hi-C experiment.")
    parser.add_argument(
        "--all_valid_pairs_stats", type=str, help="All valid pairs stats file."
    )
    parser.add_argument("--pairing_stats", type=str, help="Pairing stats file.")
    parser.add_argument(
    ...
    parser.add_argument("--fithichip_bed", type=str, help="FithiChIP bed file.")
    parser.add_argument("--fithichip_q01_bed", type=str, help="FithiChIP q01 bed file.")
    parser.add_argument("--peaks_bed", type=str, help="Peaks bed file.")
...
if __name__ == "__main__":
    args = get_args()
    FILES = [
        args.all_valid_pairs_stats,
        args.pairing_stats,
        args.mapping_stats_read1,
        args.mapping_stats_read2,
    ]
    VARIABLE = [
        "valid_interaction",
        "cis_shortRange",
    ...
    PERCENTAGES["R2_aligned"] = round(
        (RESULTS["mapped_R2"] * 100) / RESULTS["total_R2"]
    )
    PERCENTAGES["valid_interactionPairs"] = round(
        (RESULTS["valid_interaction"] * 100) / RESULTS["Reported_pairs"]
    )
    PERCENTAGES["cis_shortRange"] = round(
        (RESULTS["cis_shortRange"] * 100) / RESULTS["valid_interaction"]
    )
    PERCENTAGES["cis_longRange"] = round(
        (RESULTS["cis_longRange"] * 100) / RESULTS["valid_interaction"]
    )
