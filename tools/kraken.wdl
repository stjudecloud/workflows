...
task download_taxonomy {
    ...
    String kraken_db_name = "kraken2_taxonomy"
    ...
    command <<<
        ...
        kraken2-build --download-taxonomy \
            ...
            --use-ftp \
            --db ~{kraken_db_name} 2>&1 \
            | awk '/gunzip:/ { print; exit 42 } !/gunzip:/ { print }' 1>&2

        tar -C ~{kraken_db_name} -czf "~{kraken_db_name}.tar.gz" .

        rm -r ~{kraken_db_name}
    >>>

    output {
        File taxonomy = kraken_db_name + ".tar.gz"
    }
...
task download_library {
    ...
    String kraken_db_name = "kraken2_"+library_name+"_library"
    ...
    output {
        File library = kraken_db_name + ".tar.gz"
    }
...
task create_library_from_fastas {
    ...
    String kraken_db_name = "kraken2_custom_library"
    ...
    output {
        File custom_library = kraken_db_name + ".tar.gz"
    }
...
task build_db {
    ...
    parameter_meta {
        ...
        kraken_db_name: "Name for output in compressed, archived format. The suffix `.tar.gz` will be added."
        protein: "Construct a protein database?"
    ...
    input {
        Array[File] tarballs
        String kraken_db_name = "kraken2_db"
        Boolean protein = false
    ...
    command <<<
        ...
        while read -r tarball; do
            tar -xzf "$tarball" -C ~{kraken_db_name} --no-same-owner
        done < tarballs.txt
        ...
        kraken2-build --clean --threads "$n_cores" --db ~{kraken_db_name}
        >&2 echo "*** done ***"
    ...
    output {
        File built_db = kraken_db_name + ".tar.gz"
    }
...
task kraken {
    ...
    input {
        ...
        File db
        String prefix = basename(read_one_fastq_gz, "_R1.fastq.gz")
