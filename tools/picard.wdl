...
#PL:illumina
#PU:622BR.8
#LB:H_LC-SJRB001-X-SJ39.3-8L-lib4
#PI:309
#DS:paired end
#DT:2010-07-12T19:00:00-0500
#SM:H_LC-SJRB001-X-SJ39.3-8L
#CN:WUGSC
#PG:bwa
task add_or_replace_read_groups {
    input {
        File bam
        String output_filename = basename(bam, ".bam") + ".read_group.bam"
        String read_group_id #ID
        String read_group_library #LB
        String read_group_platform #PL
        String read_group_platform_unit #PU
        String read_group_sample_name #SM
        String? read_group_center_name #CN
        String? read_group_description #DS
        String? read_group_run_date #DT
        String? read_group_flow_order #FO
        String? read_group_key_sequence #KS
        String? read_group_program_group #PG
        String? read_group_predicted_insert_size #PI
        String? read_group_platform_model #PM
        String validation_stringency = "SILENT"
        Boolean create_index = false
        Boolean create_md5_file = false
        String? sort_order
        Int memory_gb = 25
        Int? disk_size_gb
        Int max_retries = 1
    ...
    command <<<
        picard -Xmx~{java_heap_size}g AddOrReplaceReadGroups \
            I=~{bam} \
            O=~{output_filename} \
            RGID=~{read_group_id}  \ # read group ID
            RGLB=~{read_group_library} \ # read group library
            RGPL=~{read_group_platform} \ # read group platform
            RGPU=~{read_group_platform_unit} \ # read group platform unit
            RGSM=~{read_group_sample_name} \ # read group sample name
            ~{sep="RGCN=" read_group_center_name}
            ~{sep="RGDS=" read_group_description}
            ~{sep="RGDT=" read_group_run_date}
            ~{sep="RGFO=" read_group_flow_order}
            ~{sep="RGKS=" read_group_key_sequence}
            ~{sep="RGPG=" read_group_program_group}
            ~{sep="RGPI=" read_group_predicted_insert_size}
            ~{sep="RGPM=" read_group_platform_model}
            ~{sep="SORT_ORDER=" sort_order}
    ...
    output {
        File read_group_bam = output_filename
    }
    meta {
        author: "Andrew Thrasher"
        email: "andrew.thrasher@stjude.org"
        description: "This WDL tool adds read group information to a BAM file."
    }
    parameter_meta {
        bam: "Input BAM format file to which to add read group information"
    }
}