...
task download_library {
    meta {
        description: "Downloads a predefined library of reference genomes from NCBI."
        help: "Detailed organism list for libraries (except nt) available [here](https://ftp.ncbi.nlm.nih.gov/genomes/refseq/)"
    ...
    parameter_meta {
        library_name: {
            ...
            warning: "Note that `protein` must equal `true` if downloading the `nr` library, and `protein` must equal `false` if downloading the `UniVec` or `UniVec_Core` library.",
            choices: [
                "archaea",
                "bacteria",
                "plasmid",
        ...
        protein: "Construct a protein database?"
        modify_disk_size_gb: {
            description: "Add to or subtract from dynamic disk space allocation, specified in GB. Default disk size is determined dynamically based on `library_name`.",
            warning: "Note that the default sizes are adequate as of April 2023, but new genomes are constantly being added to the NCBI database. More disk space may be required depending on when in the future this task is run.",
        }
    }
...
task create_library_from_fastas {
    ...
    parameter_meta {
        fastas_gz: {
            ...
            help: "Each FASTA sequence ID must contain either an NCBI accession number or an explicit assignment of the taxonomy ID using `kraken:taxid`",
        }
        protein: "Construct a protein database?"
...
task build_db {
    ...
    parameter_meta {
        tarballs: {
            ...
            description: "Tarballs containing the NCBI taxonomy (generated by the `download_taxonomy` task) and at least one library (generated by the `download_library` or `create_library_from_fastas` task).",
            warning: "Tarballs must not have a root directory.",
        }
        db_name: {
        ...
        max_db_size_gb: {
            description: "Maximum number of GBs for Kraken 2 hash table.",
            help: "If the Kraken 2 estimator determines more would normally be needed, the reference library will be downsampled to fit.",
        }
        ncpu: {
