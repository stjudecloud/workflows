...
task mutect2 {
    ...
    Int disk_size_gb = ceil(size(reference_fasta, "GB") * 2) + ceil(size(normal_bam, "GB")
        * 2) + ceil(size(tumor_bam, "GB") * 2) + 20 + modify_disk_size_gb
    ...
    command <<<
        ...
        rm -rf "$ref_fasta.fa" "$ref_fasta.fa.fai" "$ref_fasta.dict" "~{tumor}" "~{tumor}.bai" "~{
            normal
        }" "~{normal}.bai"
    >>>
...
task filter_mutect {
    ...
    Int disk_size_gb = ceil(size(reference_fasta, "GB") * 2) + ceil(size(
        unfiltered_somatic_vcf, "GB"
    ) * 2) + 20 + modify_disk_size_gb
    ...
    command <<<
        ...
        rm -rf "$ref_fasta.fa" "$ref_fasta.fa.fai" "$ref_fasta.dict" "~{basename(
            unfiltered_somatic_vcf
        )}" "~{basename(unfiltered_somatic_vcf_index)}" "~{basename(
            unfiltered_somatic_vcf_stats
        )}"
    >>>
...
task calculate_contamination {
    ...
    Int disk_size_gb = ceil(size(tumor_pileups, "GB") * 2) + ceil(size(normal_pileups, "GB"
    ) * 2) + 20 + modify_disk_size_gb
...
task get_pileup_summaries {
    ...
    Int disk_size_gb = ceil(size(bam, "GB") * 2) + ceil(size(intervals, "GB") * 2) + ceil(
        size(variants, "GB") * 2
    ) + ceil(size(reference_fasta, "GB") * 2) + 20 + modify_disk_size_gb
    ...
    command <<<
        ...
        rm -rf "~{name}.bam" "~{name}.bam.bai" "~{basename(intervals)}" "~{basename(
            intervals_index
        )}" "~{basename(variants)}" "~{basename(variants_index)}" "$ref_fasta.fa" "$ref_fasta.fa.fai" "$ref_fasta.dict"
    >>>
